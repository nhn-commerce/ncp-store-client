<template>
  <div id="contents" class="mypage_wrap">
    <div class="myorder_menu">
      <router-link to="/mypage/orderlist">주문내역·배송조회</router-link>
      <router-link to="/mypage/cancellist" class="is_curr">취소·반품 내역</router-link>
    </div>
    <!-- // myorder_menu -->
    ...
    <ul class="myorder_list" v-if="orders">
      <template v-if="totalCount > 0">
        ...
        <OrderItem v-for="(item, index) in orders" :key="item.orderNo" :claimNos="claimNos[index]" :item="item" :showOrderType="'CANCEL'" @showAdd="showAddPop">
        </OrderItem>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState('myorder', ['loading', 'totalCount', 'orders', 'sixMonths', 'selectTypeFlg']),
      ...mapGetters('myorder', ['claimNos']),
